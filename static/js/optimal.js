...
function setupEventDelegation() {
    ...
    // 处理下拉框变化（client, server, switch, cable）
    tbody.addEventListener('change', (e) => {
        ...
        if (groupId && field) {
            updateTestGroup(parseInt(groupId), field, select.value);
        }
    });

    // 处理所有按钮点击（Run Test, Delete, Config, 模态框内按钮）
    document.body.addEventListener('click', (e) => {
        // 处理 Run Test 按钮
        const runBtn = e.target.closest('.run-test-btn');
        if (runBtn && !runBtn.disabled) {
            const groupId = runBtn.dataset.groupId;
            if (groupId) runTest(parseInt(groupId));
            return;
        }
        ...
        if (deleteBtn) {
            ...
            if (groupId && confirm('Are you sure you want to delete this test group?')) {
                deleteTestGroup(parseInt(groupId));
            }
            return;
        }

        // 处理模态框内的全选/取消全选和保存按钮
        handleModalButtonClicks(e);
    });
}
...
function handleModalButtonClicks(e) {
    ...
    // PowerManagement 全选
    if (e.target.closest('.select-all-power')) {
    ...
    if (e.target.closest('.deselect-all-power')) {
        const powerCheckboxes = modal.querySelectorAll('.power-checkbox');
    ...
    // MTU1500 全选
    if (e.target.closest('.mtu1500-select-all')) {
    ...
    // MTU1500 取消全选
    if (e.target.closest('.mtu1500-deselect-all')) {
        const protocolCheckboxes = modal.querySelectorAll('.mtu1500-protocol-checkbox');
        const speedCheckboxes = modal.querySelectorAll('.mtu1500-speed-checkbox');
    ...
    // MTU9000 全选
    if (e.target.closest('.mtu9000-select-all')) {
    ...
    // MTU9000 取消全选
    if (e.target.closest('.mtu9000-deselect-all')) {
        const protocolCheckboxes = modal.querySelectorAll('.mtu9000-protocol-checkbox');
        const speedCheckboxes = modal.querySelectorAll('.mtu9000-speed-checkbox');
        protocolCheckboxes.forEach(cb => cb.checked = false);
        speedCheckboxes.forEach(cb => cb.checked = false);
    ...
    if (e.target.closest('.save-config-btn')) {
        const groupId = modal.querySelector('.save-config-btn')?.dataset.groupId;
        if (groupId) {
